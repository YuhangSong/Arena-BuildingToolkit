...
public abstract class TreeMazeGenerator : BasicMazeGenerator {
    ...
    GenerateMaze()
    {
        ...
        while (mCellsToVisit.Count > 0) {
            ...
            if (ctv.Column + 1 < ColumnCount &&
              !GetMazeCell(ctv.Row, ctv.Column + 1).IsVisited && !IsCellInList(ctv.Row, ctv.Column + 1))
            {
            ...
            if (ctv.Row + 1 < RowCount &&
              !GetMazeCell(ctv.Row + 1, ctv.Column).IsVisited && !IsCellInList(ctv.Row + 1, ctv.Column))
            {
    ...
    private bool
    IsCellInList(int row, int column)
    {
