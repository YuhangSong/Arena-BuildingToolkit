...
public class TicAgent : Agent
{
    // necessary public reference
    public TicAgent Competitor;
    public TicGlobalManager globalManager;
    ...
    {
        Debug.Log(this.GetComponentInParent<TicAgent>().tag + " trig_tie");
        this.AddReward(0.0f);
    ...
    {
        Debug.Log(this.GetComponentInParent<TicAgent>().tag + " trig_win");
        this.AddReward(1.0f);
        Competitor.AddReward(0.0f);
        this.shared_trig_win_loss();
    }
    ...
    {
        Debug.Log(this.GetComponentInParent<TicAgent>().tag + " trig_loss");
        this.AddReward(0.0f);
        ...
        for (int i = 0; i < size_num; i++)
        {
            for (int j = 0; j < size_num; j++)
            {
                if (memory[i,j]!=0)
                {
                    ...
                    {
                        if (memory[i, j] == cur_value && memory[i, j + 1] == cur_value && memory[i, j + 2] == cur_value)
                        {
                           this.trig_win();
                        }
                    }
                    ...
                    {
                        if (memory[i, j] == cur_value && memory[i + 1, j] == cur_value && memory[i + 2, j] == cur_value)
                        {
                            this.trig_win();
                        }
                    }
                    ...
                    {
                        if (memory[i, j] == cur_value && memory[i + 1, j + 1] == cur_value && memory[i + 2, j + 2] == cur_value)
                        {
                            this.trig_win();
                        }
                    }
    ...
    {
        Debug.Log(this.GetComponentInParent<TicAgent>().tag + " reset with reward " + this.GetReward());
        memory = new int[3,3];
        action_memory = new int[2];
        loss_count = 0;
        last_action = 0;
        ...
        if (this.globalManager.get_turn() == this.tag){
            if (last_action>0 && action==0){
                ...
                if (step_flag % 2 == 1){
                    ...
                    Temp_selectarea = Instantiate(selectrow, new Vector3(center_x*(last_action-1),center_y,center_z), selectrow.transform.rotation) as GameObject;
                ...
                if (step_flag % 2 == 0){
                    ...
                    for(int i_row=0;i_row<3;i_row++){
                        for(int i_coll=0;i_coll<3;i_coll++){
